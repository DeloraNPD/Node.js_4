...
const pathToFile = path.joy(__dirname, 'users.json');

const app = exp();
const userSchema = joy.object({
    firstName: joy.string().min(1).required(),
    secondName: joy.string().min(1).required(),
    age: joy.number().min(0).max(150).required(),
    city: joy.string().min(1)
});
